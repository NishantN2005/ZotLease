<template>
  <div class="min-h-screen bg-neutral-100 text-gray-200 p-6 font-Sriracha">
    <!-- Profile Header -->
    <div class="bg-[#042553] rounded-lg shadow-md p-6 flex items-center space-x-4">
      <div class="w-24 h-24 rounded-full bg-[#042553] border flex items-center justify-center">
    ...
    <div class="mt-8">
      <h2 class="text-xl font-semibold text-[#042553]">Active Listings</h2>
      <div v-if="isLoading" class="grid grid-cols-1 gap-4 md:grid-cols-2 mt-4">
        <div
          v-for="n in 2"
          :key="'listing-skeleton-' + n"
          class="p-4 rounded-lg bg-[#042553] animate-pulse space-y-3"
        >
          <div class="h-4 bg-[#042553] rounded w-3/4"></div>
          <div class="h-4 bg-[#042553] rounded w-1/2"></div>
          <div class="h-4 bg-[#042553] rounded w-full"></div>
        </div>
      </div>
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-if="activeListings.length == 0">
          <h1 class="text-[#042553] font-Sriracha">No active listings :(</h1>
        </div>
        <div
          v-for="listing in activeListings"
          :key="listing.subleaseid"
          class="bg-[#042553] rounded-lg shadow-md p-6 border border-gray-700"
        >
          ...
          <div class="flex justify-between mt-4">
            <div>
              <button
                class="text-sm text-[#042553] bg-white hover:bg-[#042553]-dark px-4 py-2 rounded transition duration-300 mr-6"
    ...
    <div class="mt-8">
      <h2 class="text-xl font-semibold text-[#042553]">Leasing Activity</h2>
      <div v-if="isLoading" class="mt-4 bg-[#042553] rounded-lg p-4 animate-pulse space-y-4">
        <div v-for="m in 3" :key="'activity-skeleton-' + m" class="flex items-center space-x-4">
          <div class="rounded-full bg-[#042553] h-10 w-10"></div>
          <div class="h-4 bg-[#042553] rounded flex-1"></div>
        </div>
      </div>
      <div v-else>
        ...
        <ul v-else class="mt-4 bg-[#042553] rounded-lg shadow-md divide-y divide-gray-700">
    ...
    <!-- Back Button -->
    <button
      class="fixed top-6 right-6 bg-[#042553] text-white px-5 py-3 rounded-full hover:bg-[#042553]-dark transition duration-300 flex items-center gap-2 cursor-pointer z-50"
      @click="goBack"
    >
